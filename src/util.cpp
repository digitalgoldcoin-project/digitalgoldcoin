...
const char * const BITCOIN_CONF_FILENAME = "digitalgoldcoin.conf";
const char * const BITCOIN_PID_FILENAME = "digitalgoldcoin.pid";
...
#else
    const char* pszModule = "digitalgoldcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DigitalGoldCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DigitalGoldCoin";
#else
    // Unix
    return pathRet / ".digitalgoldcoin";
#endif
#endif
}
