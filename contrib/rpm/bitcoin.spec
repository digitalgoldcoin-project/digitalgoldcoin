...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16781
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16783
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 26781
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 26783
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 16781
	%{_sbindir}/semanage port -d -p tcp 16783
	%{_sbindir}/semanage port -d -p tcp 26781
	%{_sbindir}/semanage port -d -p tcp 26783
	for selinuxvariant in %{selinux_variants}; do
